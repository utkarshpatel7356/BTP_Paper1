...
Architecture:
  ...
  1. For each time step t in [t-W, t]:
       h_t = GAT(x_t, edge_index, edge_weight)    -> (N+1, hidden)
...
class GATTemporalEncoder(nn.Module):
    ...
    def forward(
        self,
        x: torch.Tensor,          # (N+1, T, F) or (B, N+1, T, F) — supports batch
        ...
        gat_outs = []  # will be (T, N+1, hidden)
        attn_weights_last = None

        for t in range(max(0, T - self.gru_window), T):
            x_t = x[:, t, :]  # (N+1, F)
            h = x_t
            for i, layer in enumerate(self.gat_layers):
                if i < len(self.gat_layers) - 1:
                    ...
                    h, (ei, attn) = layer(h, edge_index, return_attention_weights=True)
                    attn_weights_last = attn  # (E, heads)
                h = self.act(h)
                h = self.dropout(h)
            h = self.gat_norm(h)
        ...
        return embeddings, attn_weights_last
...
class SparseIndexGNN(nn.Module):
    ...
    def forward(self, x, edge_index, edge_weight=None):
        ...
        """
        embeddings, attn_weights = self.encoder(x, edge_index, edge_weight)
        y_hat = self.head(embeddings[-1]).squeeze()  # last node = index sink
        return y_hat, embeddings, attn_weights
